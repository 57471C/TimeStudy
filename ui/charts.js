...
const openCompareDashboard = () => {
	...
	for (const trial of trials) {
		...
		if (trial.metrics) {
			va = trial.metrics.va || 0;
			nva = trial.metrics.nva || 0;
			w = trial.metrics.w || 0;
		} else {
		...
		const units =
			totalMs > 0 ? Math.floor((effectiveMs / totalMs) * cycleUnits) : 0;
	...
	setTimeout(() => {
		const vaNvaChart = new ApexCharts(
			document.getElementById("compareVaNvaChart"),
			{
				series: [
		...
		const unitsChart = new ApexCharts(
			document.getElementById("compareUnitsChart"),
			{
		...
		const costChart = new ApexCharts(
			document.getElementById("compareCostChart"),
			{
					...
					events: {
						beforeZoom: (chartContext, { xaxis }) => {
							...
							if (newMin <= boundaryMin && newMax >= boundaryMax) {
								if (
									chartContext.w.globals.minX <= boundaryMin &&
									chartContext.w.globals.maxX >= boundaryMax
								) {
									return {
										xaxis: {
			...
			if (groupingMode === "lean") {
				...
				for (let j = 0; j < op.tasks.length; j += 1) {
					if (op.tasks[j]?.status) {
						statusDurations[op.tasks[j].status.toUpperCase()] +=
							op.tasks[j].duration;
					}
				}
				pieData = [
				...
				pieData = Object.entries(labourDurations)
					.map(([code, duration]) => ({
						name: code,
						y: duration,
