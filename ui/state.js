...
const saveLocalState = () => {
	...
	trials[activeTrialIndex].costingConfig = {
		hourlyRate,
		shiftLength,
		targetEfficiency,
		unitsPerCycle,
	...
	trials[activeTrialIndex].metrics = { va, nva, w };
...
const loadLocalState = () => {
	...
	if (data) {
		try {
			...
			if (state.projectMeta) {
				partsList =
					state.projectMeta.partsList || state.projectMeta.masterParts || [];
				labourList =
					state.projectMeta.labourList || state.projectMeta.masterLabour || [];
			...
			if (state.appConfig) {
				...
				volumeLevel =
					state.appConfig.volumeLevel !== undefined
						...
						: 1;
				autoGenerateVTT =
					state.appConfig.autoGenerateVTT !== undefined
						? state.appConfig.autoGenerateVTT
						: false;
				autoLoadVTT =
			...
			if (state.trials && state.trials.length > 0) {
				trials = state.trials;
				activeTrialIndex =
					state.activeTrialIndex !== undefined ? state.activeTrialIndex : 0;
				...
				if (currentTrial) {
					...
					targetEfficiency =
						currentTrial.costingConfig?.targetEfficiency || 100;
	...
	if (DOM.autoGenerateVTTInput)
		DOM.autoGenerateVTTInput.checked = autoGenerateVTT;
...
const importFromJSON = (jsonText) => {
	try {
		...
		if (data.trials) {
			...
			partsList =
				data.projectMeta?.partsList ||
				data.projectMeta?.masterParts ||
				data.partsList ||
				data.masterParts ||
				[];
			labourList =
				data.projectMeta?.labourList ||
				data.projectMeta?.masterLabour ||
				data.labourList ||
				data.masterLabour ||
				[];
			...
			trials = [
				{
					...
					appState: {
						operations: data.operations || data.appState?.operations || [],
					},
				},
			];
			activeTrialIndex = 0;
			...
			projectComments =
				data.projectComments || data.projectMeta?.projectComments || "";
			partsList =
				data.partsList ||
				data.projectMeta?.partsList ||
				data.masterParts ||
		...
		toConsole(
			...
			`Loaded Trial: ${currentTrial.trialName}`,
			debuggin,
		);
	...
	for (let i = 0; i < operations.length; i += 1) {
		...
		if (partTags.length === 0) {
			...
			for (let pIdx = 1; pIdx < partTags.length; pIdx += 1) {
				const { qty, partNumber, partDescription } = parsePartTag(
					partTags[pIdx],
				);
		...
		// Task Titles
		csvContent +=
			"Task Name,Task Labour Code,Task Labour Description,VA,NVA,W,Total Task Time\n";
		...
		for (let j = 0; j < op.tasks.length; j += 1) {
			...
			const valVA =
				status === "VA" ? formatDecimalMinutes(task.duration) : "0.00";
			const valNVA =
				status === "NVA" ? formatDecimalMinutes(task.duration) : "0.00";
			const valW =
				status === "W" ? formatDecimalMinutes(task.duration) : "0.00";
	...
	for (let tIdx = 0; tIdx < trials.length; tIdx += 1) {
		...
		for (let i = 0; i < trialOps.length; i += 1) {
			...
			for (let j = 0; j < op.tasks.length; j += 1) {
				...
				const valVA =
					status === "VA" ? parseFloat(formatDecimalMinutes(task.duration)) : 0;
				const valNVA =
					status === "NVA"
						? parseFloat(formatDecimalMinutes(task.duration))
						: 0;
				const valW =
					status === "W" ? parseFloat(formatDecimalMinutes(task.duration)) : 0;
	...
	try {
		...
		for (const trial of projectData.trials) {
			if (trial.videoFileName && trial.videoFileName.trim() !== "") {
				...
				const baseName =
					lastDot !== -1
						? trial.videoFileName.substring(0, lastDot)
						: trial.videoFileName;
				...
				if (vttContent && vttContent.trim() !== "") {
					await window.__TAURI__.fs.writeTextFile(vttPath, vttContent, {
						append: false,
					});
				}
			}
