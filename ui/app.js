...
const addTrial = async () => {
	const trialName = await asyncPrompt(
		"Enter a name for the new trial:",
	...
	saveLocalState();
	const newTrialId =
		trials.length > 0 ? Math.max(...trials.map((t) => t.trialId)) + 1 : 1;

	const newTrial = duplicate
		? {
				...JSON.parse(JSON.stringify(trials[activeTrialIndex])),
				trialId: newTrialId,
				trialName,
			}
		: {
...
const processNewVideoFile = async (fileOrPath, isTauriPath = false) => {
	...
	const isRelinking =
		!hasExistingVideo && (operations.length > 0 || projectName !== "");
...
const initializePlayer = () => {
	...
	if (DOM.openSettingsBtn) {
		const saveSettingsData = () => {
			...
			targetEfficiency =
				Number.parseFloat(DOM.targetEfficiencyInput.value) || 100;
			...
			if (DOM.taktTimeInput) {
				...
				if (parsedTakt !== null) {
					...
					alert(
						"Invalid Takt Time format. Please use HH:MM:SS.MS (e.g., 00:01:00.00).",
					);
			...
			if (DOM.autoGenerateVTTInput)
				autoGenerateVTT = DOM.autoGenerateVTTInput.checked;
	...
	player.addEventListener("play", () => {
		...
		if (window.syncTimelinePlayheadSmoothly)
	...
	player.addEventListener("pause", () => {
		...
		if (window.cancelAnimationFrame && window.playheadAnimationId)
	...
	if (DOM.opContextMenu) {
		DOM.opRenameBtn.addEventListener("click", () => {
			if (
				currentOpContextIndex !== null &&
				currentOpContextIndex !== undefined
			) {
	...
	document.addEventListener("click", (e) => {
		if (
			!DOM.timeContextMenu.contains(e.target) &&
			e.target !== DOM.currentTime
		) {
	...
	const performStandardSave = async () => {
		...
		if (projectFilePath && projectFilePath.endsWith(".tspz")) {
			...
			const destFolder = await window.__TAURI__.dialog.open({
				directory: true,
			});
			if (!destFolder) return;
			const destPath =
				typeof destFolder === "object" ? destFolder.path : destFolder;
			...
			try {
				...
				const newVideoPaths = await window.__TAURI__.core.invoke(
				...
				const cleanDest = destPath.endsWith(separator)
					? destPath
					: destPath + separator;
	...
	if (projectSaveAsButton) {
		projectSaveAsButton.addEventListener(
			"click",
			() => handleProjectSave(false),
			false,
		);
	}
	if (projectPackageButton) {
		projectPackageButton.addEventListener(
			"click",
	...
	if (projectExportButton) {
		projectExportButton.addEventListener(
			"click",
			() => performStandardSave(),
			false,
		);
					...
					if (projectFilePath.endsWith(".tspz")) {
						...
						if (response.video_paths && response.video_paths.length > 0) {
							response.video_paths.forEach((vPath) => {
								const filename = vPath.split(/[/\\]/).pop();
								trials.forEach((trial) => {
									if (trial.videoFileName === filename)
										trial.videoFilePath = vPath;
								});
							});
							const currentTrial = trials[activeTrialIndex];
							if (currentTrial?.videoFilePath) {
								player.src = window.__TAURI__.core.convertFileSrc(
									currentTrial.videoFilePath,
								);
								player.load();
							}
						}
	...
	if (btnLoadExample) {
		btnLoadExample.addEventListener("click", async () => {
			...
			try {
				...
				const resourcePath = await window.__TAURI__.path.resolveResource(
					"_up_/demo/Example.tspz",
				);
				...
				alert(
					`Failed to load example project: ${err.message || JSON.stringify(err)}`,
				);
			}
		});
	}
...
window.onload = () => {
	...
	const hasActiveWorkspace =
		videoFilePath || videoFileName || (operations && operations.length > 0);
		...
		window.__TAURI__.core
			...
			.then(async (startupFile) => {
				if (!startupFile) {
					...
					if (sessionActive) {
						toConsole(
							"Startup file empty/null but session is active. Aborting clean workspace reset.",
							null,
							debuggin,
						);
						return;
				...
				if (startupFile) {
					...
					try {
						...
						if (startupFile.endsWith(".tspz")) {
							...
							const jsonText =
								await window.__TAURI__.fs.readTextFile(startupFile);
							importFromJSON(jsonText);
						}
						saveLocalState();
						toConsole(
							"Auto-loaded project from file association",
							startupFile,
							debuggin,
						);
					} catch (e) {
...
const endMarquee = (event) => {
	...
	window.translateX = translateX =
		wrapperWidth / 2 - (marqueeCenterX - oldX) * actualZoomFactor;
	window.translateY = translateY =
		wrapperHeight / 2 - (marqueeCenterY - oldY) * actualZoomFactor;
...
const addOp = async () => {
	...
	if (startTime < processStartTime) {
		showToast(
			`Operation "${opName}" starts before Process Start Time.`,
			"error",
		);
...
const addTask = async () => {
	...
	const taskName = await asyncPrompt(
	...
	let taskStatus = await asyncPrompt(
		"VA, NVA, W? (or 1=VA, 2=NVA, 3=W)",
		"",
		"Task Status",
	);
	if (!taskStatus) {
...
const splitOperationAt = async (opIndex, taskIndex) => {
	...
	const remainingDurationMs = originalOp.tasks.reduce(
...
const deleteTask = async (opIndex, taskIndex) => {
	if (
		...
		if (
			operations[opIndex].tasks.length === 0 &&
...
const deleteOperation = async (opIndex) => {
	if (
		...
		toConsole(
			...
			`Total ops left: ${operations.length}`,
			debuggin,
		);
		saveLocalState();
		updateTaskList();
...
const jumpToTaskTime = (event, opIndex, taskIndex) => {
	event.stopPropagation();
	const op = operations[opIndex];
	if (!op) return;
	const precedingDuration = op.tasks
		.slice(0, taskIndex)
		.reduce((sum, t) => sum + (t.duration || 0), 0);
	const taskStartTime = op.startTime + precedingDuration / 1000;

	if (player.src) {
		player.currentTime = taskStartTime;
		player.play();
		toConsole("Jumped to task time", taskStartTime, debuggin);
	} else {
		alert("Please load a video first.");
	}
};
...
const initializeTrimFeature = () => {
	...
	const handleTrimAction = async (isCompression) => {
		...
		const startVal = parseTimeFromHHMMSSMS(
			document.getElementById("trimStartInput").value,
		);
		const endVal = parseTimeFromHHMMSSMS(
			document.getElementById("trimEndInput").value,
		);

		if (startVal === null || endVal === null) {
			toConsole(
				"Trim validation failed: Invalid time format",
				{ startVal, endVal },
				debuggin,
			);
		...
		if (startVal >= endVal) {
			toConsole(
				"Trim validation failed: Start >= End",
				{ startVal, endVal },
				debuggin,
			);
		...
		const qualityMode = document.querySelector(
			'input[name="trimQuality"]:checked',
		).value;
		toConsole(
			"Trim parameters validation success",
			{ startVal, endVal, qualityMode },
			debuggin,
		);
	...
	if (trimOnlyBtn)
		trimOnlyBtn.addEventListener("click", () => handleTrimAction(false));
	if (trimCompressBtn)
		trimCompressBtn.addEventListener("click", () => handleTrimAction(true));
};
...
const processVideo = async (start, end, qualityMode, isCompression) => {
	...
	if (!videoFilePath) {
		toConsole(
			"processVideo abort: No active video file path found",
			null,
			debuggin,
		);
	...
	if (
		...
		toConsole(
			"processVideo abort: Input and output paths are identical",
			actualOutputPath,
			debuggin,
		);
