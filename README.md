...
- [Node.js](https://nodejs.org/) (optional, for linting/formatting with ESLint and Prettier).
...
3. **(Optional) Install Development Tools**:
   To ensure code quality, install ESLint and Prettier:
   ```bash
   npm install
   ```
   This assumes a `package.json` with ESLint/Prettier configured (see [Development](#development)).
...
TimeStudy uses the Airbnb JavaScript style guide with Prettier formatting:
...
- Screenshots or logs (enable `debuggin = 1` in `index.html` for detailed logs).
